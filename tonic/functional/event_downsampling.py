...
def differentiator_downsample(
    events: np.ndarray,
    sensor_size: tuple,
    target_size: tuple,
    dt: float,
    differentiator_time_bins: int = 2,
    ...
    dt_scaling, events_integrated = integrator_downsample(
        events,
        sensor_size=sensor_size,
        target_size=target_size,
        dt=(dt / differentiator_time_bins),
        noise_threshold=noise_threshold,
        differentiator_call=True,
    )
    ...
    events_new = np.column_stack(
        (x_new, y_new, polarity_new.astype(dtype=bool), time_index * dt)
    )
...
def integrator_downsample(
    ...
    all_frame_histograms = to_frame_numpy(
        events, sensor_size=(*target_size, 2), time_window=dt
    )
    ...
    for time, frame_histogram in enumerate(frame_histogram_diffs):
        ...
        coordinates_pos = np.stack(
            np.nonzero(np.maximum(frame_spike >= noise_threshold, 0))
        ).T
        coordinates_neg = np.stack(
            np.nonzero(np.maximum(-frame_spike >= noise_threshold, 0))
        ).T
        ...
        if np.logical_or(coordinates_pos.size, coordinates_neg.size).sum():
            ...
            events_new.append(
                np.column_stack(
                    (
                        np.flip(coordinates_pos, axis=1),
                        np.ones((coordinates_pos.shape[0], 1)).astype(dtype=bool),
            ...
            events_new.append(
                np.column_stack(
                    (
                        np.flip(coordinates_neg, axis=1),
                        np.zeros((coordinates_neg.shape[0], 1)).astype(dtype=bool),
